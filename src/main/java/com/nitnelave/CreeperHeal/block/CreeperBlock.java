...
public class CreeperBlock implements Replaceable {
   ...
   */
  protected final static Set<Material> EMPTY_BLOCKS = CreeperUtils.createFinalHashSet(Material.AIR, Material.WATER, Material.STATIONARY_WATER,
      Material.LAVA, Material.STATIONARY_LAVA, Material.FIRE, Material.SNOW);
  /*
   * These blocks (may) need a block under them not to drop.
   */
  private final static Set<Material> DEPENDENT_DOWN_BLOCKS = CreeperUtils.createFinalHashSet(Material.SAPLING, Material.BED_BLOCK,
   ...
   */
  private final static Set<Material> DEPENDENT_BLOCKS = CreeperUtils.createFinalHashSet(Material.TORCH, Material.LADDER, Material.WALL_SIGN,
      Material.LEVER,
  ...
  public BlockFace getAttachingFace() {
    if (blockState.getData() instanceof Attachable) { return ((Attachable) blockState.getData()).getAttachedFace(); }
